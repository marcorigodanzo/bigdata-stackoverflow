...
object StackOverflow extends StackOverflow {
  ...
  @transient lazy val conf: SparkConf = new SparkConf().setMaster("local").setAppName("StackOverflow") //.setLogLevel("ERROR")
...
class StackOverflow extends Serializable {
  ...
  def scoredPostings(grouped: RDD[(Int, Iterable[(Posting, Posting)])]): RDD[(Posting, Int)] = {
    ...
    grouped.map{
      //case (id:Int , it:Iterable[(Posting,Posting)]) => (it.head._1 , answerHighScore(it.map(pair => pair._2).toArray) )
      x => ( x._2.head._1 ,   answerHighScore(x._2.map(pair => pair._2).toArray) )
    }
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(Int, Int)]): Array[(String, Double, Int, Int)] = {
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      // list of labels,#num
      val langLabels = vs.groupBy(x => x._1).mapValues(y => y.size)

      // gets the label and returns to the original index
